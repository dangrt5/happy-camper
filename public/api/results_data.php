...
if( $city !== '' ){
    ...
    $query = "SELECT id, park_name, addr, phone, park_desc, weather_overview, direction_info, regulation_info, park_website, SQRT( POW(69.1 * (lat - {$lat}), 2) + POW(69.1 * ({$lng} - lng) * COS(lat / 57.3), 2)) AS distance FROM park_info HAVING distance < '$radius' order by park_name";
}
// print_r($query);
